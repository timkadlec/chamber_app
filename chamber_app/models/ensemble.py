...
ensemble_composition = db.Table('ensemble_composition',
    ...
    db.Column('status', db.String(50), nullable=False, default="studying")  # Track work status (e.g., studying, completed)
)
...
class Ensemble(db.Model):
    ...
    teacher_id = db.Column(db.Integer, db.ForeignKey('teachers.id'))  # Link to Teacher

    # Relationship to Composition
    composition = relationship('Composition', backref='ensembles', lazy=True)

    # Relationship to Teacher
    teacher = relationship('Teacher', backref='ensembles', lazy=True)
    ...
    def player_completeness(self):
        total_players = self.composition.player_count  # Assuming this gives the total required players
        assigned_players = self.count_assigned_students
        if total_players == 0:
            return "No players required"  # Handle case where no players are required
        return (assigned_players / total_players) * 100  # Return percentage with two decimal places
